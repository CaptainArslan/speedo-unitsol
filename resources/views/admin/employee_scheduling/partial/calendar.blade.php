    ...
    ! function(NioApp, $) {
        ...
        var calendarEvents = [
            @foreach ($records as $schedule)
                {
                    id: '{{ $schedule->id }}',
                    title: '{{ $schedule?->class->name .' - '.$schedule?->name}}',
                    startTime: '{{ $schedule?->timing->start_time }}',
                    endTime: '{{ $schedule?->timing->end_time }}',
                    backgroundColor: '{{ $schedule?->class->color }}',
                    color: '{{ $schedule?->class->color }}',
                    className: "{{ $schedule->class->color }}",
                    class: "{{ $schedule?->class->name }}",
                    venue: "{{ $schedule?->venue->name }}",
                    trainer: "{{ $schedule?->user?->first_name.' '.$schedule?->user?->last_name  }}",
                    s_date: "{{ date('F d, Y', strtotime($schedule->start_date)) }}",
                    e_date: "{{ date('F d, Y', strtotime($schedule->end_date)) }}",
                    daysOfWeek: @json($schedule->dayId()),
                },
            @endforeach
        ]
        ...
        NioApp.Calendar = function() {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                timeZone: 'UTC',
                initialView: mobileView ? 'timeGridWeek' : 'timeGridWeek',
                ...
                views: {
                    week: {
                        dayMaxEventRows: calendarEvents.length
                    }
                },
                ...
                eventClick: function eventClick(info) {
                    ...
                    var class_name = info.event._def.extendedProps.class;
                    // var description = info.event._def.extendedProps.description;
                    var s_date = info.event._def.extendedProps.s_date;
                    var e_date = info.event._def.extendedProps.e_date;
                    var venue = info.event._def.extendedProps.venue;
                    var trainer = info.event._def.extendedProps.trainer;
                    ...
                    scedule_id = info.event._def.publicId; // Set data in preview
                    ...
                    $('#preview-event-class').text(class_name);
                    $('#preview-event-venue').text(venue);
                    $('#preview-event-trainer').text(trainer);
                    $('#preview-event-s_date').text(s_date);
                    $('#preview-event-e_date').text(e_date);
            ...
            editEventPopup.on("click", function(e) {
                e.preventDefault();
                var url = '{{ url('admin/term-base-bookings') }}/' + scedule_id + '/edit'
                window.location = (url)
            });
            deleteEventBtn.on("click", function(e) {
                e.preventDefault();
                var url = '{{ url('admin/term-base-bookings') }}/' + scedule_id
                deleteRecordAjax(url)
            });
