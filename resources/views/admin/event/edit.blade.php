                                        ...
                                        <div class="row g-3">
                                                        ...
                                                        <select id="event-theme" name="venue_id" required
                                                            ...
                                                            @foreach ($venues as $item)
                                                                <option value="{{ $item->id }}"
                                                                    @if ($item->id == $event->venue_id) selected @endif>
                                                                    {{ $item->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="form-control-wrap">
                                                        <input type="text" name="google_map_location"
                                                            class="form-control" value="{{ $event->google_map_location }}"
                                                            id="product-title">
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-12">
                                                ...
                                                <div class="card card-preview">
                                                    <div class="card-inner">
                                                        <select id="basic-listbox" name="students[]" class="dual-listbox"
                                                            multiple>
                                                            @foreach ($students as $item)
                                                           
                                                                <option value="{{ $item->id }}"
                                                                     @foreach ($event->eventStudents as $student)
                                                                    @if ($student->student_id == $item->id) checked @endif 
                                                                     @endforeach >
                                                                    {{ $item->name }}
                                                                </option>
                                            ...
                                            <div class="col-4">
                                                <button type="button"
                                                    onclick="addFormData(event,'post','{{ url('admin/events/' . $event->id) }}','{{ url('admin/events') }}','add-event')"
                                                    class="btn btn-primary"><em
                                                        class="icon ni ni-edit"></em><span>Update</span></button>
                                                <button type="button"
                                                    onclick="window.location='{{ url('admin/events') }}'"
...
@section('scripts')
    ...
    <script>
        $('.summernote-basic').summernote({
            ...
            tabsize: 2,
            height: 100,
        }).summernote('code', `{!! $event->description !!}`);

        var li = @json($event->eventStudents);
        let listing = '';
        console.log(li)
        ...
        li.forEach(element => {
            console.log(element)
            listing +=
                '<li class="dual-listbox__item dual-listbox__item--selected" data-id="1" draggable="false" data-order="0">' +
                element.name + '</li>'
        });
        $('.dual-listbox__selected').append(listing);
