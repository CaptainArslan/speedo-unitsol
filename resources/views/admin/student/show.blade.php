                                ...
                                <div class="col-lg-3">
                                    <div class="product-gallery mr-xl-1 mr-xxl-5">
                                        <div class="slider-item rounded">
                                            <img src="{{ $image_url . '/' . $student?->image }}" class="w-100"
                                                alt="">
                                        </div>
                                ...
                                <div class="col-lg-9">
                                    <div class="product-info mt-1 mr-xxl-5">
                                        ...
                                        <div class="col-3 float-right" id="showCredit" style="display: none">
                                            <form id="credit">
                                                <input type="number" name="credit" value="{{ $student?->credit }}"
                                                    class="form-control mb-2">
                                            </form>
                                            <button type="button"
                                                onclick="addStudentCredit(event,'{{ url('admin/add_student_credit/' . $student?->id) }}','credit')"
                                                class="btn btn-sm btn-primary float-right">Add Credit</button>
                                        </div>
                                        <div class="col-3 mt-1 float-right" id="hideCredit">
                                            <h6>Credit: {{ $student?->credit }} <a href='#' id="addCredit"
                                                    class='toggle float-right' data-target='editClass'><em
                                                        class='icon ni bold ni-plus'></em></a></h6>
                                        ...
                                        <div class="col-3 float-right ">
                                            <div class="form-group">
                                                ...
                                                <div class="form-control-wrap">
                                                    <select name="status"
                                                        onchange="changeAssessmentStatus(event,'{{ url('admin/change_assessment_status/' . $student?->id) }}')"
                                                        class=" form-control" data-search="on">
                                                        <option value="">change status</option>
                                                        <option value='Active'
                                                            @if ($student?->assessmentRequest?->status == 'Active') selected @endif>Active
                                                        </option>
                                                        <option value='Call back requested'
                                                            @if ($student?->assessmentRequest?->status == 'Call back requested') selected @endif>Call back
                                                            requested
                                                        </option>
                                                        <option value='Follow up 1'
                                                            @if ($student?->assessmentRequest?->status == 'Follow up 1') selected @endif>Follow up 1
                                                        </option>
                                                        <option value='Follow up 2'
                                                            @if ($student?->assessmentRequest?->status == 'Follow up 2') selected @endif>Follow up 2
                                                        </option>
                                                        <option value='Lost'
                                                            @if ($student?->assessmentRequest?->status == 'Lost') selected @endif>Lost</option>

                                                    </select>
                                                </div>
                                            </div>
                                        </div>
                                        <h2 class="product-title">{{ $student?->name }}</h2>

                                        <a href="{{ url('admin/customer-informations/' . $student?->user?->id) }}"
                                            class="btn btn-success">Parent Info</a>
                                            ...
                                            <ul class="d-flex g-3 gx-5">
                                                ...
                                                <li>
                                                    ...
                                                    <div class="fs-14px fw-bold text-secondary">
                                                        {{ date('M m,Y', strtotime($student?->dob)) }}</div>
                                                </li>
                                                <li>
                                                ...
                                                <div class="row g-3">
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <div class="form-control-wrap">
                                                                <input type="date" name="till_date" required
                                                                    class="form-control"
                                                                    value="{{ $assesment_request->till_date }}"
                                                                    id="product-title">
                                                            </div>
                                                        </div>
                                                    </div> --}}
                                                    ...
                                                    <div class="col-4">
                                                        <div class="form-group">
                                                            ...
                                                            <div class="form-control-wrap">
                                                                <button type="button"
                                                                    onclick="addFormData(event,'post','{{ url('admin/assessment-requests/' . $assesment_request?->id) }}','{{ url('admin/students/' . $student?->id) }}','edit-class')"
                                                                    class="btn btn-primary"><em
                                                                        class="icon ni ni-edit"></em><span>Confirm</span></button>
                                                            </div>
                                                        </div>
                                                    </div>
