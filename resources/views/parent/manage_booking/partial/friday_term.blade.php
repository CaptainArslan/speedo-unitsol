    ...
    <?php
     ...
     @foreach ($group_term as $parent_term)
         ...
         if (isset($days_id)) {
             ...
             $day = $parent_term->termBaseBookingDays->where('day_id', 5)->first();
         }
             ...
             @forelse ($child_terms as $term)
                 ...
                 @if ($term->timing)
                     <div class="us-class-list box" id="us-scroll{{ $term->id }}" onclick="scrollToDiv(event,'{{ $term->id }}')"
                         ...
                         data-toggle="modal" data-target="#displayClassFridayDetail{{ $term->id }}">
                 ...
                 @endif
                 <div class="modal modal_close" id="displayClassFridayDetail{{ $term->id }}">
                                                                 ...
                                                                 @if ($student_id != [])
                                                                     @if (is_null($booked_term))
                                                                         @if ($total_slot < $term->no_of_student)
                                                                             <button type="button"
                                                                                 @if ($term_check == false) onclick="addToCart(event,'{{ url('parent/add-to-cart-term/' . $term->id . '-' . $student_record?->student_id.'-'.'Friday') }}')" @endif
